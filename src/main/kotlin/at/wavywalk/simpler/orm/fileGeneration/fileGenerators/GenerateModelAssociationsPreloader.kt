...
class GenerateModelAssociationsPreloader(val modelDataModel: ModelDataModel) {

    fun run(){
        val file: File = GeneratedFileFactory.createDefault(
                packageName = modelDataModel.packagesBean.baseGenerated,
                fileName = "${modelDataModel.modelClass}AssociationsPreloader.kt"
        )

        val templateName: String = TemplateFilePathsConfig.modelAssociationsPreloaderTemplate
