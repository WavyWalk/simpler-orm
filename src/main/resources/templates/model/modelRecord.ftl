...
class ${modelClass}Record(val model: ${modelClass}) {
    ...
    fun save(dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext){
    ...
    fun saveCascade(dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext){
        if (!validationManager.isValid()) {
            throw ModelInvalidError("failed saving ${r"${model}"} of ${modelClass}: validation failed")
        }
        if (isPersited()) {
    ...
    inline fun save(before: (${modelClass}Record)->Unit, after: (${modelClass}Record)->Unit, dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext) {
        before.invoke(this)
        save(dslContext)
        after.invoke(this)
    }

    inline fun saveCascade(before: (${modelClass}Record)->Unit, after: (${modelClass}Record)->Unit, dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext) {
        before.invoke(this)
        saveCascade(dslContext)
        after.invoke(this)
    }

    private fun saveCascadeDependentOnUpdate(dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext) {
        <#list associationBeans as ab>
        <#if ab.associationType == "HAS_ONE">
    ...
    private fun saveCascadeDependentOnCreate(dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext) {
        <#list associationBeans as ab>
        <#if ab.associationType == "HAS_ONE">
        model.${ab.propertyName}?.let {
            it.record.${ab.fieldOnThat.property} = model.${ab.fieldOnThis.property}
    ...
    fun delete(dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext){
        if (!validationManager.isValid()) {
    ...
    inline fun delete(before: (${modelClass}Record)->Unit, after: (${modelClass}Record)->Unit, dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext) {
        before.invoke(this)
        delete(dslContext)
        after.invoke(this)
    }

    fun updateChanged(dslContext: DSLContext = SimplerOrmDependenciesManager.provider.defaultDslContext) {
